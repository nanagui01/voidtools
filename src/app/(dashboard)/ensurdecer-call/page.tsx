...
import { VolumeX, Volume2, Loader2, CheckCircle2, AlertTriangle, X, RotateCcw } from "lucide-react"
...
export default function PaginaEnsurdecerCall() {
  const { activeToken } = useTokens()
  const [channelId, setChannelId] = useState("")
  const [ensurdecer, setEnsurdecer] = useState(true)
  ...
  const handleStart = async () => {
    ...
    try {
      const res = await api.runTool("call-utils", {
        ...
        action: ensurdecer ? "deafen-all" : "undeafen-all",
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card/40 p-6">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">Ensurdecer / Desensurdecer Call</h2>
              <p className="text-xs text-muted-foreground">Ensurdece ou desensurdece todos os membros de um canal de voz</p>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button variant={ensurdecer ? "default" : "outline"} onClick={() => setEnsurdecer(true)} className={`flex-1 h-10 ${ensurdecer ? "bg-orange-600 text-white hover:bg-orange-700" : ""}`}>
                  <VolumeX size={16} className="mr-2" /> Ensurdecer Todos
                </Button>
                <Button variant={!ensurdecer ? "default" : "outline"} onClick={() => setEnsurdecer(false)} className={`flex-1 h-10 ${!ensurdecer ? "bg-green-600 text-white hover:bg-green-700" : ""}`}>
                  <Volume2 size={16} className="mr-2" /> Desensurdecer
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
            <Button onClick={handleStart} disabled={starting || !activeToken || !channelId.trim()} className={`h-11 w-full text-white ${ensurdecer ? "bg-orange-600 hover:bg-orange-700" : "bg-green-600 hover:bg-green-700"}`}>
              {starting ? <Loader2 size={18} className="mr-2 animate-spin" /> : ensurdecer ? <VolumeX size={18} className="mr-2" /> : <Volume2 size={18} className="mr-2" />}
              {ensurdecer ? "Ensurdecer Todos" : "Desensurdecer Todos"}
            </Button>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">{resultado?.afetados} membros {resultado?.action === "deafened" ? "ensurdecidos" : "desensurdecidos"}</p>
            </div>
