...
export default function PaginaSairServidores() {
  ...
  useEffect(() => {
    api.getTasks().then(res => {
      ...
      const running = tasks.find((t: any) => t.status === 'running' && t.tool === 'remover-servidores')
      if (running) {
  ...
  const handleStart = async () => {
    ...
    setError("")
    try {
      const res = await api.runTool("remover-servidores", {
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card/40 p-6">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">Sair de Servidores</h2>
              <p className="text-xs text-muted-foreground">Sai de todos os servidores da sua conta Discord</p>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-lg border border-orange-500/20 bg-orange-500/5 p-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-orange-400">
                <strong>Atenção:</strong> Esta ação é irreversível. Você sairá de todos os servidores.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6">
            <Button onClick={handleStart} disabled={starting || !activeToken} className="h-11 w-full bg-orange-600 text-white hover:bg-orange-700">
              {starting ? <Loader2 size={18} className="mr-2 animate-spin" /> : <LogOut size={18} className="mr-2" />}
        ...
        <div className="rounded-xl border border-border/50 bg-card/20 backdrop-blur-sm p-6">
          <div className="flex items-center justify-between mb-6 pb-6 border-b border-border/50">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-orange-500/10">
                <LogOut size={18} className="text-orange-400 animate-pulse" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-foreground">Saindo dos Servidores</h3>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">{progress} de {total} servidores • {fmt(elapsedSeconds)}</p>
            </div>
            {statusMessage && (
        ...
        <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">Saiu de {progress} servidores em {fmt(elapsedSeconds)}</p>
            </div>
